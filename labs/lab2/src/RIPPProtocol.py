...
class RIPPProtocol(StackingProtocol):
    ...
    def sendSyn(self):
        synPacket = RIPPPacket.makeSynPacket(self.initialSeq)
    ...
    def sendSynAck(self, synAck_seqNum):
        synAckPacket = RIPPPacket.makeSynAckPacket(synAck_seqNum, self.associatedSeqNum)
    ...
    def sendAck(self):
        ackPacket = RIPPPacket.makeAckPacket(self.associatedSeqNum)
        print("Sending ACK packet with ack number " + str(self.associatedSeqNum) + ", current state " + self.STATE_DESC[self.state])
        self.transport.write(ackPacket.__serialize__())

    def sendFin(self):
        finPacket = RIPPPacket.makeFinPacket(self.seqNum, self.associatedSeqNum)
    ...
    def sendFinAck(self):
        finAckPacket = RIPPPacket.makeFinAckPacket(self.associatedSeqNum)
    ...
    def processDataPkt(self, pkt):
        ...
        ackPacket = RIPPPacket.makeAckPacket(AckNo)
